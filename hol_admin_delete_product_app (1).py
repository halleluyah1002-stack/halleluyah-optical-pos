...
def pos():
    ...
    if view == "lenses":
        ...
        recent_lenses = SaleItem.query.filter(SaleItem.lens_power != "").order_by(SaleItem.id.desc()).limit(5).all()
    ...
    <div class="sales-layout">
        <div class="sales-left">
            ...
            <div class="card">
                ...
                <input id="powerSearch" placeholder="🔍 Search power e.g. -2.00, +1.50, cyl, add" onkeyup="filterPowerCards()"><br><br>
                {% if recent_lenses %}<div class="alert success"><b>Recent Lens Sales:</b>{% for r in recent_lenses %}<span style="display:inline-block;background:white;padding:6px 10px;border-radius:10px;margin:4px;color:#0f172a">{{ r.product_name }} - {{ r.lens_power }}</span>{% endfor %}</div>{% endif %}
                <div class="pos-card-grid">
                    {% for p in product_list %}
    ...
    function filterPowerCards(){const q=document.getElementById('powerSearch').value.toLowerCase();document.querySelectorAll('.lens-product-card').forEach(card=>{card.style.display=card.innerText.toLowerCase().includes(q)?'block':'none';});}
    ...
    """, view=view, lens_category=lens_category, lens_material=lens_material, lens_categories=lens_categories, lens_materials=lens_materials, product_list=product_list, lens_map=lens_map, lens_stock_map=lens_stock_map, recent_lenses=recent_lenses, cart=cart, cart_total=cart_total, customer_name=customer_name, customer_phone=customer_phone, price_type=price_type, selected_branch_id=selected_branch_id, short_lens_name=short_lens_name, stock_badge=stock_badge)
