...
def page(title, content, **context):
    ...
    """, Branch=Branch, User=User, Product=Product, LensPower=LensPower,
       Sale=Sale, SaleItem=SaleItem, OfficeSetting=OfficeSetting, GoodsRequest=GoodsRequest, session=session,
       money=money, **context)
...
def home():
    ...
    <div class="card"><h2>Menu</h2><nav>
        ...
        <a href="/add-product" class="btn-green">Add Product</a><a href="/add-lens-power" class="btn-green">Add Lens Power</a>
        <a href="/restock" class="btn-green">Restock</a><a href="/generate-power-grid" class="btn-green">Generate Power Grid</a>
        <a href="/office-settings" class="btn-gray">Office Settings</a><a href="/branches">Branches</a><a href="/staff">Staff Accounts</a><a href="/goods-requests" class="btn-gray">Goods Requests</a>
        {% endif %}
        <a href="/pos" class="btn-gold">Make Sale</a><a href="/request-goods" class="btn-green">Request Goods</a><a href="/sales">Sales History</a><a href="/debtors">Debtors</a><a href="/logout" class="btn-red">Logout</a>
    </nav></div>
    """
...
def lens_powers():
    ...
    {% for l in power_list %}<tr><td>{{ branches_map.get(l.branch_id, '') }}</td><td>{{ products_map.get(l.product_id, '') }}</td><td>{{ l.sph }}</td><td>{{ l.cyl }}</td><td>{{ l.axis }}</td><td>{{ l.add_power }}</td><td>{% if l.quantity <= 2 %}<span class="badge-low">LOW</span> {% endif %}{{ l.quantity }}</td>{% if session['role']=='manager' %}<td><a href="/restock-lens/{{ l.id }}">Restock</a></td>{% endif %}</tr>{% endfor %}
    </table></div><a class="btn" href="/">Back</a>
...
def pos():
    ...
    # American-standard POS rule:
    # Manager can sell from any branch; staff can sell only from their own branch.
    if is_manager():
        product_list = Product.query.filter(Product.quantity > 0).order_by(Product.category, Product.name).all()
    else:
        product_list = Product.query.filter(
            Product.branch_id == session.get("branch_id"),
            Product.quantity > 0
    ...
    if request.method == "POST":
        ...
        price_type = request.form.get("price_type", "retail")
        ...
        for product in product_list:
            ...
            # LENS PRODUCTS: allow many powers under one product, e.g. -0.25, -0.50, +5.00 for one customer.
            if is_lens_product(product):
                powers = LensPower.query.filter_by(product_id=product.id).filter(LensPower.quantity > 0).all()

                for lp in powers:
                    qty = int(request.form.get(f"lens_qty_{lp.id}") or 0)
                    if qty <= 0:
                        continue
                    ...
                    if qty > int(lp.quantity or 0):
                        return page(
                            ...
                            <div class='card'>
                                ...
                                <a class='btn' href='/pos'>Go Back</a>
                            </div>
                            """,
                            product=product,
                            lp=lp,
                    ...
                    lens_text = f"SPH {lp.sph or ''} CYL {lp.cyl or ''} AXIS {lp.axis or ''} ADD {lp.add_power or ''}".strip()
                    subtotal = price * qty
                    total += subtotal
                    ...
                    product_deductions[product.id] = product_deductions.get(product.id, 0) + qty

            # NORMAL PRODUCTS: frames, case, cleaner, accessories.
            else:
                ...
                subtotal = price * qty
                total += subtotal
                selected_items.append((product.name, "", qty, price, subtotal, None))
                product_deductions[product.id] = product_deductions.get(product.id, 0) + qty

        if not selected_items:
            return page(
                "No Product Selected",
                "<div class='card'><h2>No Product Selected</h2><p>Please enter quantity for at least one product or lens power.</p><a class='btn' href='/pos'>Back</a></div>"
            )

        # Check total product stock after all selected powers are added together.
        for product_id, deduct_qty in product_deductions.items():
            product = Product.query.get(product_id)
            if product and deduct_qty > int(product.quantity or 0):
                return page(
                    "Insufficient Product Stock",
                    """
                    <div class='card'>
                        <h2>Insufficient Product Stock</h2>
                        <p>Total selected powers exceed the main product stock.</p>
                        <p><b>Product:</b> {{ product.name }}</p>
                        <p><b>Available:</b> {{ product.quantity }}</p>
                        <p><b>Requested:</b> {{ deduct_qty }}</p>
                        <a class='btn' href='/pos'>Go Back</a>
                    </div>
                    """,
                    product=product,
        ...
        # Proper stock subtraction after sale is safely created.
        for name, lens_text, qty, price, subtotal, lp in selected_items:
            ...
            if lp:
                lp.quantity = max(0, int(lp.quantity or 0) - qty)

        for product_id, deduct_qty in product_deductions.items():
            product = Product.query.get(product_id)
            if product:
                product.quantity = max(0, int(product.quantity or 0) - deduct_qty)
    ...
    <div class="card">
        ...
        <p class="small">American-standard optical checkout: select multiple lens powers under one lens product. Use Qty 2 for a pair and Qty 1 for half pair.</p>
        ...
        <form method="post">
            ...
            <h3>Products and Lens Powers</h3>
            <table>
                <tr>
                    <th>Product</th>
                    <th>Available Stock</th>
                    <th>Power Selection / Quantity</th>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        {% if 'Lens' in p.category %}
                            <p class="small"><b>Select powers for this lens.</b> Qty 2 = pair, Qty 1 = half pair.</p>
                            <table>
                                <tr><th>Power</th><th>Available</th><th>Qty to Sell</th></tr>
                                ...
                                {% else %}
                                <tr><td colspan="3">No available power entered for this lens.</td></tr>
                                {% endfor %}
                            </table>
                        {% else %}
    ...
    """, product_list=product_list, lens_map=lens_map)
...
def debtors():
    ...
    return page("Debtors", """
    <div class="card"><h2>Debtors</h2><table><tr><th>Date</th><th>Customer</th><th>Phone</th><th>Total</th><th>Paid</th><th>Balance</th></tr>{% for s in debtor_list %}<tr><td>{{ s.created_at }}</td><td>{{ s.customer_name }}</td><td>{{ s.customer_phone }}</td><td>{{ money(s.total) }}</td><td>{{ money(s.amount_paid) }}</td><td>{{ money(s.balance) }}</td></tr>{% endfor %}</table></div><a class="btn" href="/">Back</a>
