...
.dark{background:#0f172a;color:#e5e7eb;min-height:100vh}.dark .card{background:#111827;color:#e5e7eb;box-shadow:0 4px 15px #0006}.dark table{background:#111827}.dark td{border-color:#374151}.dark input,.dark select,.dark textarea{background:#0b1220;color:#e5e7eb;border-color:#475569}.dark .small{color:#cbd5e1}.dark .header{background:linear-gradient(90deg,#020617,#1e3a8a)}.dark .stat{background:#111827}.thermal-receipt{max-width:320px;margin:auto;font-family:Arial,monospace;font-size:12px;color:#000;background:white}.thermal-receipt table{font-size:11px}.thermal-receipt hr{border:0;border-top:1px dashed #000}
@media print{nav,.btn,button,.no-print{display:none}.card{box-shadow:none}body{background:white}.header{background:white;color:black;padding:0}.container{padding:0}.thermal-receipt{max-width:80mm}.thermal-receipt *{color:#000!important}}
</style>
"""
...
def page(title, content, **context):
    ...
    <div class="card no-print" style="padding:12px;margin-bottom:15px">
        <nav>
            ...
            <a href="/patient-rx">Patient Rx</a>
            <a href="/frame-try-on">Frame Try-On</a>
            ...
            <a href="/generate-power-grid" class="btn-green">Generate Power Grid</a>
            ...
            <a href="/audit-trail" class="btn-gray">Audit Trail</a>
...
def lens_restock_center():
    ...
    if request.method == "POST":
        ...
        for key, value in request.form.items():
            ...
            if lens and int(lens.branch_id or 0) == selected_branch:
                ...
                updated += 1
        db.session.commit()
        audit_log("Bulk Lens Restock", f"Bulk restocked {updated} lens powers in branch ID {selected_branch}")
        message = f"Updated {updated} lens powers successfully."
    ...
    <p class="small">Select a branch, filter the lens powers, enter quantities, and save all at once.</p>
    {% if message %}<div class="alert success"><b>{{ message }}</b></div>{% endif %}
    ...
    <form method="post">
        ...
        <div class="alert success no-print">
            <b>Fast entry:</b> Use +1, +2, +5, +10, +20, or type the exact number of lenses beside each power. Then click <b>Save All Restock</b> once.
        </div>
        <table><tr><th>Lens</th><th>SPH</th><th>CYL</th><th>AXIS</th><th>ADD</th><th>Current Qty</th><th>Add Number of Lenses</th><th>Quick Add</th></tr>
        {% for l in lens_rows %}<tr>
            <td>{{ products_map.get(l.product_id, '') }}</td>
            <td>{{ l.sph }}</td><td>{{ l.cyl }}</td><td>{{ l.axis }}</td><td>{{ l.add_power }}</td>
            <td>{% if l.quantity <= 2 %}<span class="badge-low">LOW</span> {% endif %}{{ l.quantity }}</td>
            <td><input id="add_qty_{{ l.id }}" type="number" name="add_qty_{{ l.id }}" min="0" value="0" style="max-width:130px;font-size:18px;font-weight:bold;text-align:center;padding:12px"></td>
        ...
        </tr>{% else %}<tr><td colspan="8">No lens powers found for this branch/filter.</td></tr>{% endfor %}
        </table><br>
        <button class="btn-green" type="submit" style="font-size:18px;padding:15px 24px">Save All Restock</button>
    </form></div>
    <script>
    function setQty(id, value){
        const input = document.getElementById(id);
    ...
    window.addEventListener('load', function(){
        const first = document.querySelector('input[name^="add_qty_"]');
...
def generate_power_grid():
    ...
    product_list = Product.query.filter(Product.branch_id == int(selected_branch), Product.category.contains("Lens")).order_by(Product.name).all()
    message = ""
    if request.method == "POST":
        product = Product.query.get(int(request.form.get("product_id")))
        ...
        if product and int(product.branch_id or 0) == int(selected_branch):
            ...
            for sph in sph_values:
                for add_val in add_values:
                    ...
                    existing = LensPower.query.filter_by(product_id=product.id, branch_id=int(selected_branch), sph=sph_s, cyl="", axis="", add_power=add_s).first()
                    if existing:
                        if default_qty:
                            existing.quantity = int(existing.quantity or 0) + default_qty
                        updated += 1
                    else:
                        db.session.add(LensPower(product_id=product.id, branch_id=int(selected_branch), sph=sph_s, cyl="", axis="", add_power=add_s, quantity=default_qty))
                        created += 1
            db.session.commit()
            audit_log("Power Grid Generated", f"Branch ID {selected_branch}, Product {product.name}, Created {created}, Updated {updated}")
            message = f"Created {created} powers and updated {updated} existing powers for the selected branch."
        else:
            message = "Please select a valid lens product under the selected branch."
    return page("Generate Power Grid", """
    {% if message %}<div class="alert success"><b>{{ message }}</b></div>{% endif %}
    <div class="card"><h2>Generate Lens Power Grid</h2><p>This lets you choose branch first, then generate powers only for lens products inside that branch.</p>
    <form method="get" class="no-print">
        <label>Select Branch First</label><br>
        <select name="branch_id" onchange="this.form.submit()" required>{% for b in branches %}<option value="{{ b.id }}" {% if selected_branch|int == b.id %}selected{% endif %}>{{ b.name }}</option>{% endfor %}</select>
    </form>
    <hr>
    <form method="post">
        <input type="hidden" name="branch_id" value="{{ selected_branch }}">
        <label>Select Lens Product In Selected Branch</label><br><select name="product_id" required>{% for p in product_list %}<option value="{{ p.id }}">{{ p.name }} - {{ p.category }} - {{ p.subcategory }}</option>{% else %}<option value="">No lens product found in this branch</option>{% endfor %}</select><br><br>
        ...
        <label>Default Quantity for each generated power</label><br><input name="default_qty" type="number" value="0"><br><br><button>Generate Powers For Selected Branch</button>
    </form></div><a class="btn" href="/">Back</a>
    """, product_list=product_list, message=message, branches=branches, selected_branch=selected_branch)
...
def pos():
    ...
    def selected_product_query():
        # BRANCH SAFETY FIX:
        # Product, lens power and sales must stay inside one selected branch.
        # Manager can choose a branch, but products from different branches must never mix together.
        # Staff can only sell from their own assigned branch.
        selected_branch_id = int(
            request.form.get("branch_id")
            or session.get("pos_customer", {}).get("branch_id")
        ...
        base_query = Product.query.filter(Product.branch_id == selected_branch_id)

        if view == "lenses":
            # Lens product quantity is controlled by exact LensPower stock.
            return base_query.filter(Product.category.contains("Lens")).order_by(Product.category, Product.name).all()
        if view == "frames":
            # Frames/accessories use Product.quantity.
    ...
    if request.method == "POST":
        ...
        selected_branch_id = int(request.form.get("branch_id") or session.get("pos_customer", {}).get("branch_id") or session.get("branch_id") or 1)
        if not is_manager():
            selected_branch_id = int(session.get("branch_id") or 1)
        ...
        for product in product_list:
            ...
            if is_lens_product(product):
                powers = LensPower.query.filter_by(product_id=product.id, branch_id=product.branch_id).filter(LensPower.quantity > 0).all()

                # Automatic Pair Helper: choose OD and OS powers; system adds one lens for each eye.
                auto_pair_ids = []
                od_id = request.form.get(f"auto_od_{product.id}")
                os_id = request.form.get(f"auto_os_{product.id}")
                if od_id:
                    auto_pair_ids.append((od_id, "OD / Right Eye"))
                if os_id:
                    auto_pair_ids.append((os_id, "OS / Left Eye"))
                for lp_id, eye_label in auto_pair_ids:
                    lp = LensPower.query.get(int(lp_id))
                    if not lp or lp.product_id != product.id:
                        continue
                    requested_for_same_power = sum(1 for x, _label in auto_pair_ids if str(x) == str(lp_id))
                    if requested_for_same_power > int(lp.quantity or 0):
                        return page("Insufficient Lens Power Stock", """
                        <div class='card'>
                            <h2>Insufficient Lens Power Stock</h2>
                            <p>The automatic pair helper selected more quantity than available.</p>
                        ...
                        """, lp=lp)
                    lens_text = f"Auto Pair {eye_label} | SPH {lp.sph or ''} CYL {lp.cyl or ''} AXIS {lp.axis or ''} ADD {lp.add_power or ''}".strip()
                    cart.append({
                        "type": "lens",
    ...
    lens_map = {
        p.id: LensPower.query.filter_by(product_id=p.id, branch_id=p.branch_id).filter(LensPower.quantity > 0).order_by(LensPower.sph, LensPower.cyl, LensPower.add_power).all()
        ...
        <form method="post">
            ...
            {% if view == 'lenses' %}
                ...
                <div class="alert success">
                    <b>Automatic Pair Helper:</b> For one customer pair, choose OD power and OS power under the same lens product. The system adds one lens for OD and one lens for OS automatically.
                </div>
            {% elif view == 'frames' %}
                ...
                <tr>
                    ...
                    <td>
                        {% if 'Lens' in p.category %}
                            <div class="card" style="background:#eef6ff;margin-bottom:10px">
                                <b>Automatic Pair Helper for {{ p.name }}</b><br>
                                <div class="two">
                                    <div>
                                        <label>OD / Right Eye Power</label>
                                        <select name="auto_od_{{ p.id }}">
                                            <option value="">Not selected</option>
                                            {% for l in lens_map[p.id] %}<option value="{{ l.id }}">SPH {{ l.sph }} CYL {{ l.cyl }} AXIS {{ l.axis }} ADD {{ l.add_power }} | Qty {{ l.quantity }}</option>{% endfor %}
                                        </select>
                                    </div>
                                    <div>
                                        <label>OS / Left Eye Power</label>
                                        <select name="auto_os_{{ p.id }}">
                                            <option value="">Not selected</option>
                                            {% for l in lens_map[p.id] %}<option value="{{ l.id }}">SPH {{ l.sph }} CYL {{ l.cyl }} AXIS {{ l.axis }} ADD {{ l.add_power }} | Qty {{ l.quantity }}</option>{% endfor %}
                                        </select>
                                    </div>
            ...
            <form method="post" action="/pos/checkout">
                <div class="two">
                    <div>
                        ...
                        <p class="small">Discount requires manager PIN below.</p>
                    </div>
                    <div>
                ...
                <br>
                <label>Manager PIN for Discount Approval</label><br>
                <input name="manager_pin" type="password" placeholder="Required only if discount is entered"><br><br>
...
def receipt(sale_id):
    ...
    <p><b>Address:</b> {{ setting.address }}</p><p><b>Phone:</b> {{ setting.phone }}</p><p><b>Branch:</b> {{ branch.name if branch else '' }}</p><p><b>Attended By:</b> {{ attended_by.username if attended_by else 'Unknown' }}</p><p><b>Date:</b> {{ sale.created_at }}</p><p><b>Customer:</b> {{ sale.customer_name }}</p><p><b>Customer Phone:</b> {{ sale.customer_phone }}</p>
    ...
    <p><b>Discount:</b> {{ money(sale.discount) }}</p><p><b>Total:</b> {{ money(sale.total) }}</p><p><b>Amount Paid:</b> {{ money(sale.amount_paid) }}</p><p><b>Balance:</b> {{ money(sale.balance) }}</p><p><b>Change Due:</b> {{ money(change_due) }}</p><p><b>Payment:</b> {{ sale.payment_method }}</p><button onclick="window.print()">Print Receipt</button> <a class="btn-green btn" href="/receipt/{{ sale.id }}/thermal">80mm Thermal Receipt</a></div><a class="btn" href="/">Back</a>
...
def thermal_receipt(sale_id):
    ...
    <div class="thermal-receipt">
        ...
        <hr>
        <center>Thank you for choosing HOL<br>Your vision is our priority</center>
...
@app.route("/frame-try-on")
def frame_try_on():
    if not logged_in():
        return redirect(url_for("login"))
    return page("Frame Try-On", """
    <div class="card">
        <h2>Frame Try-On Camera</h2>
        <p class="small">Use the webcam to preview frame shape on a customer's face. This works best in Chrome/Edge and may require camera permission.</p>
        <div class="two">
            <div>
                <label>Frame Shape</label><br>
                <select id="shape" onchange="drawFrame()">
            ...
            <div>
                <label>Frame Color</label><br>
                <select id="color" onchange="drawFrame()">
        ...
        <div class="two">
            <div><label>Frame Width</label><input id="fw" type="range" min="180" max="420" value="300" oninput="drawFrame()"></div>
            <div><label>Frame Position Up/Down</label><input id="fy" type="range" min="80" max="330" value="190" oninput="drawFrame()"></div>
        </div><br>
        <div class="two">
            <div><label>Frame Position Left/Right</label><input id="fx" type="range" min="120" max="520" value="320" oninput="drawFrame()"></div>
            <div><label>Lens Height</label><input id="fh" type="range" min="50" max="130" value="82" oninput="drawFrame()"></div>
        </div><br>

        <div style="position:relative;max-width:720px;margin:auto;background:#111;border-radius:16px;overflow:hidden">
            <video id="video" autoplay playsinline style="width:100%;display:block"></video>
            <canvas id="overlay" width="640" height="480" style="position:absolute;left:0;top:0;width:100%;height:100%"></canvas>
        </div>
        <br>
        <div class="no-print" style="text-align:center">
            <button onclick="startCamera()">Start Camera</button>
            <button onclick="stopCamera()" class="btn-red">Stop Camera</button>
            <button onclick="autoFit()" class="btn-gold">Auto Fit Guide</button>
            <button onclick="captureTryOn()" class="btn-green">Capture Preview</button>
            <button onclick="window.print()" class="btn-gray">Print Preview</button>
        </div>
        <div id="captureBox" style="text-align:center;margin-top:20px"></div>
    </div>
    ...
    async function startCamera(){
        ...
        }catch(e){ alert('Camera could not start. Allow camera permission or use HTTPS/localhost.'); }
    }
    function stopCamera(){
    ...
    function drawFrame(){
        ...
        const w=Number(document.getElementById('fw').value);
        const h=Number(document.getElementById('fh').value);
        const color=document.getElementById('color').value;
        const shape=document.getElementById('shape').value;
        const bridge=28, lw=w/2-bridge;
        ctx.strokeStyle=color; ctx.lineWidth=8; ctx.lineCap='round'; ctx.lineJoin='round';
        function lens(cx, cy){
            ctx.beginPath();
            if(shape==='round'){ ctx.ellipse(cx,cy,lw/2,h/2,0,0,Math.PI*2); }
            else if(shape==='square'){ ctx.roundRect(cx-lw/2,cy-h/2,lw,h,14); }
            else if(shape==='cat'){ ctx.moveTo(cx-lw/2,cy); ctx.quadraticCurveTo(cx-lw/3,cy-h/2,cx+lw/3,cy-h/2); ctx.quadraticCurveTo(cx+lw/2+18,cy-h/2-15,cx+lw/2,cy); ctx.quadraticCurveTo(cx+lw/3,cy+h/2,cx-lw/3,cy+h/2); ctx.quadraticCurveTo(cx-lw/2-18,cy-h/2-15,cx-lw/2,cy); }
            else if(shape==='aviator'){ ctx.moveTo(cx-lw/2,cy-h/3); ctx.quadraticCurveTo(cx,cy-h/2,cx+lw/2,cy-h/3); ctx.quadraticCurveTo(cx+lw/2,cy+h/2,cx,cy+h/2); ctx.quadraticCurveTo(cx-lw/2,cy+h/2,cx-lw/2,cy-h/3); }
            else { ctx.ellipse(cx,cy,lw/2,h/2,0,0,Math.PI*2); ctx.setLineDash([12,8]); }
            ctx.stroke(); ctx.setLineDash([]);
        }
        lens(x-w/4-bridge/2,y); lens(x+w/4+bridge/2,y);
        ctx.beginPath(); ctx.moveTo(x-bridge/2,y); ctx.quadraticCurveTo(x,y-18,x+bridge/2,y); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(x-w/2-8,y-5); ctx.lineTo(x-w/2-90,y-40); ctx.moveTo(x+w/2+8,y-5); ctx.lineTo(x+w/2+90,y-40); ctx.stroke();
    }
    function autoFit(){
        document.getElementById('fx').value=320; document.getElementById('fy').value=190; document.getElementById('fw').value=310; document.getElementById('fh').value=82; drawFrame();
        alert('Auto Fit Guide applied. Adjust left/right or up/down if needed.');
    }
    function captureTryOn(){
        ...
        const ctx=out.getContext('2d'); ctx.drawImage(video,0,0,out.width,out.height); ctx.drawImage(overlay,0,0,out.width,out.height);
        document.getElementById('captureBox').innerHTML='<h3>Captured Frame Preview</h3><img style="max-width:100%;border-radius:14px;border:1px solid #ddd" src="'+out.toDataURL('image/png')+'">';
    }
