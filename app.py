...
def restock_lens(lens_id):
    ...
    <p>Current Quantity in this branch: {{ lens.quantity }}</p>
    <form method="post">
        ...
        <label>Quantity to Add</label><br><input name="add_qty" type="number" min="1" placeholder="Quantity to add" required><br><br>
        <button>Add Stock To Selected Branch</button>
...
def lens_restock_center():
    ...
    <form method="post">
        ...
        <table><tr><th>Lens</th><th>SPH</th><th>CYL</th><th>AXIS</th><th>ADD</th><th>Current Qty</th><th>Add Qty</th></tr>
        {% for l in lens_rows %}<tr><td>{{ products_map.get(l.product_id, '') }}</td><td>{{ l.sph }}</td><td>{{ l.cyl }}</td><td>{{ l.axis }}</td><td>{{ l.add_power }}</td><td>{% if l.quantity <= 2 %}<span class="badge-low">LOW</span> {% endif %}{{ l.quantity }}</td><td><input type="number" name="add_qty_{{ l.id }}" min="0" value="0" style="max-width:100px"></td></tr>{% else %}<tr><td colspan="7">No lens powers found for this branch/filter.</td></tr>{% endfor %}
        </table><br>
        <button class="btn-green" type="submit">Save All Restock</button>
    </form></div>
