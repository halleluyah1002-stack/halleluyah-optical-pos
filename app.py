...
def pos():
    ...
    # Clean American-standard POS screen:
    # First show only two big choices, then load either lenses or frames/accessories.
    ...
    if is_manager():
        base_query = Product.query.filter(Product.quantity > 0)
    else:
    ...
    if view == "lenses":
        product_list = base_query.filter(
    ...
    if request.method == "POST":
        ...
        for product in product_list:
            ...
            # LENS PRODUCTS: support Half Pair, 1 Pair, and Custom Qty under the same product.
            ...
            # NORMAL PRODUCTS: frames, case, cleaner, rope, cloth, accessories.
            else:
    ...
    <div class="card">
        ...
        <p class="small">Start by choosing what you want to sell. This keeps the POS clean and fast.</p>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:16px;margin:20px 0">
            ...
            <a href="/pos?view=frames" style="background:#16a34a;color:white;padding:28px;border-radius:16px;text-decoration:none;font-size:24px;font-weight:bold;text-align:center;box-shadow:0 6px 18px #0002">
                🟩 Load Frames / Accessories<br><span style="font-size:14px;font-weight:normal">Frames, Cases, Rope, Cloth, Cleaner</span>
            </a>
        </div>
        ...
        <div class="alert">
            ...
            <p>Click <b>Load Lenses</b> to sell lenses with powers, or click <b>Load Frames / Accessories</b> to sell general optical products.</p>
        </div>
        {% endif %}
    ...
    """, product_list=product_list, lens_map=lens_map, view=view)
