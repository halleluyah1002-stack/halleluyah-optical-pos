...
def pos():
    ...
    """, product_list=product_list, lens_map=lens_map, view=view, cart=cart, cart_total=cart_total, customer=customer)
...
def pos_checkout():
    ...
    for item in cart:
        ...
        if item.get("type") == "lens":
            lp = LensPower.query.get(int(item.get("lens_power_id")))
            if lp:
                lp.quantity = max(0, int(lp.quantity or 0) - int(item.get("quantity") or 0))
