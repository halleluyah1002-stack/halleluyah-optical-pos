...
def pos():
    ...
    return page("POS", """
    <div class="card"><h2>Make Sale - Multiple Products</h2><form method="post"><input name="customer_name" placeholder="Customer Name"><br><br><input name="customer_phone" placeholder="Customer Phone"><br><br>
        ...
        <table><tr><th>Product</th><th>Stock</th><th>Lens Power</th><th>Qty</th></tr>{% for p in product_list %}<tr><td>{{ p.name }}<br><small>{{ money(p.retail_price) }} retail / {{ money(p.wholesale_price) }} wholesale</small></td><td>{{ p.quantity }}</td><td><select name="lens_{{ p.id }}"><option value="">No lens power</option>{% for l in lens_map[p.id] %}<option value="{{ l.id }}">SPH {{ l.sph }} CYL {{ l.cyl }} AXIS {{ l.axis }} ADD {{ l.add_power }} | Qty {{ l.quantity }}</option>{% endfor %}</select></td><td><input type="number" name="qty_{{ p.id }}" min="0" value="0"></td></tr>{% endfor %}</table><br>
        <input name="discount" type="number" step="0.01" value="0" placeholder="Discount"><br><br><input name="amount_paid" type="number" step="0.01" placeholder="Amount Paid"><br><br><select name="payment_method"><option>Cash</option><option>Transfer</option><option>POS</option><option>Credit</option></select><br><br><button>Complete Sale</button>
    </form></div><a class="btn" href="/">Back</a>
...
def receipt(sale_id):
    ...
    <p><b>Discount:</b> {{ money(sale.discount) }}</p><p><b>Total:</b> {{ money(sale.total) }}</p><p><b>Amount Paid:</b> {{ money(sale.amount_paid) }}</p><p><b>Balance:</b> {{ money(sale.balance) }}</p><p><b>Payment:</b> {{ sale.payment_method }}</p><button onclick="window.print()">Print Receipt</button></div><a class="btn" href="/">Back</a>
    """, sale=sale, items=items, branch=branch, setting=setting)
