...
def generate_power_grid():
        ...
        if product and int(product.branch_id or 0) == int(selected_branch):
            ...
            for sph in sph_values:
                ...
                for cyl_s in cyl_values:
                    # If no cylinder is generated, keep axis blank. If cylinder is generated, use selected axis rule.
                    ...
                    for axis_s in current_axis_values:
                        for add_val in add_values:
                            ...
                            if existing:
                                ...
                                created += 1
            db.session.commit()
            audit_log("Power Grid Generated", f"Branch ID {selected_branch}, Product {product.name}, Cylinder {generate_cylinder}, Created {created}, Updated {updated}")
    ...
    {% if message %}<div class="alert success"><b>{{ message }}</b></div>{% endif %}
    <div class="card"><h2>Generate Lens Power Grid</h2><p>This page can generate SPH powers only, or SPH + CYL powers, for lens products inside the selected branch.</p>
    <form method="get" class="no-print">
    ...
    <form method="post">
        <input type="hidden" name="branch_id" value="{{ selected_branch }}">
        <label>Select Lens Product In Selected Branch</label><br>
        <select name="product_id" required>{% for p in product_list %}<option value="{{ p.id }}">{{ p.name }} - {{ p.category }} - {{ p.subcategory }}</option>{% else %}<option value="">No lens product found in this branch</option>{% endfor %}</select><br><br>
        ...
        <div class="card" style="background:#eef6ff">
            ...
            <p class="small">Use this for toric/cylinder lens stock. Example CYL -0.25 to -6.00 with 0.25 step.</p>
