...
def pos():
    ...
    <style>
        ...
        .pos-card-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:15px}.pos-item-card{border:1px solid #dbe4ee;border-radius:16px;padding:16px;background:#fff;box-shadow:0 3px 10px #0001}.floating-cart{position:sticky;top:10px;border:2px solid #198754;background:#f8fff9}.dark .pos-item-card{background:#111827;border-color:#374151}.dark .floating-cart{background:#0b1220}
    </style>
    ...
    <div class="two">
        ...
        <div>
            <div class="card floating-cart">
                <h3>🛒 Current Sale</h3>
                {% if cart %}<table><tr><th>Item</th><th>Qty</th><th>Total</th><th></th></tr>{% for item in cart %}<tr><td>{{ item.product_name }}<br><small>{{ item.lens_power }}</small></td><td>{{ item.quantity }}</td><td>₦{{ money(item.subtotal) }}</td><td><a class="btn-red btn" href="/pos/remove/{{ loop.index0 }}">X</a></td></tr>{% endfor %}</table><h2>Total: ₦{{ money(cart_total) }}</h2><a class="btn-red btn" href="/pos/clear">Clear Cart</a>{% else %}<div class="alert">No item in cart yet.</div>{% endif %}
            </div>
            {% if cart %}<div class="card" style="border:2px solid #198754"><h3>Checkout</h3><form method="post" action="/pos/checkout"><label>Discount</label><input name="discount" type="number" step="0.01" value="0"><label>Amount Paid</label><input name="amount_paid" type="number" step="0.01" placeholder="Amount Paid"><label>Manager PIN for Discount</label><input name="manager_pin" type="password" placeholder="Only if discount is used"><label>Payment Method</label><select name="payment_method"><option>Cash</option><option>Transfer</option><option>POS</option><option>Split Payment</option><option>Credit</option></select><br><br><button class="btn-green" type="submit">Complete Sale / Print Receipt</button></form></div>{% endif %}
        </div>
    </div>
