...
class Product(db.Model):
    ...
    end_user_price = db.Column(db.Float, default=0)
...
STYLE = """
<style>
:root{--primary:#0f4c81;--primary-dark:#003366;--success:#16a34a;--danger:#dc2626;--warning:#f59e0b;--gray:#475569;--light:#f4f7fb;--white:#ffffff;--soft:#eef6ff;--border:#dbe4ee;--ink:#0f172a}*{box-sizing:border-box}body{margin:0;font-family:Arial,Helvetica,sans-serif;background:var(--light);color:var(--ink)}.header{background:linear-gradient(90deg,#003366,#0077b6);color:white;padding:22px 26px;box-shadow:0 4px 14px #0002}.header h1{margin:0;font-size:34px;letter-spacing:.5px}.header p{margin:7px 0 0;opacity:.92}.container{padding:20px}.card{background:white;border-radius:18px;padding:20px;margin-bottom:20px;box-shadow:0 4px 18px #00000010}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:18px}.stat{background:linear-gradient(135deg,#fff,#eaf6ff);border-left:6px solid var(--primary)}.stat h1{margin-top:10px;font-size:36px}
/* Professional grouped navigation */
.nav-shell{padding:16px;margin-bottom:18px}.nav-group-title{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;color:#64748b;margin:8px 0 7px}.nav-section{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid #e2e8f0}.nav-section:last-child{border-bottom:0;margin-bottom:0;padding-bottom:0}nav a,.btn{display:inline-flex;align-items:center;justify-content:center;background:var(--primary);color:white;padding:10px 14px;border-radius:12px;text-decoration:none;font-size:14px;font-weight:700;transition:.2s ease;border:none;line-height:1.1;min-height:40px}nav a:hover,.btn:hover{transform:translateY(-2px);opacity:.95}.btn-green{background:var(--success)!important}.btn-red{background:var(--danger)!important}.btn-gold{background:var(--warning)!important}.btn-gray{background:var(--gray)!important}.btn-purple{background:#7c3aed!important}.btn-blue{background:#0f4c81!important}
input,select,textarea{padding:12px;width:100%;max-width:450px;border:1px solid #cbd5e1;border-radius:10px;margin-top:5px;box-sizing:border-box;background:white;font-size:14px}input:focus,select:focus,textarea:focus{outline:none;border-color:#0077b6;box-shadow:0 0 0 3px rgba(0,119,182,.15)}button{background:var(--primary);color:white;border:0;padding:12px 18px;border-radius:10px;cursor:pointer;font-weight:700;transition:.2s}button:hover{opacity:.95}table{width:100%;border-collapse:collapse;background:white}th{background:var(--primary-dark);color:white}td,th{padding:10px;border:1px solid #e2e8f0;text-align:left;vertical-align:top}.alert{background:#fff3cd;border-left:6px solid var(--warning);padding:14px;border-radius:10px;margin-bottom:15px}.danger{background:#ffe5e5;border-left:6px solid var(--danger)}.success{background:#e8fff2;border-left:6px solid var(--success)}.small{font-size:13px;color:#64748b}.badge-low{background:var(--danger);color:white;border-radius:12px;padding:3px 8px;font-size:12px}.two{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:18px}
/* Professional POS layout */
.sales-layout{display:grid;grid-template-columns:minmax(0,1.55fr) minmax(360px,.9fr);gap:22px;align-items:start}.checkout-panel{position:sticky;top:18px;align-self:start}.current-sale-box{max-height:350px;overflow-y:auto;overflow-x:hidden;border-radius:12px;padding-right:6px}.cart-table-wrap{max-height:235px;overflow-y:auto;border:1px solid #dbe4ee;border-radius:12px}.cart-table-wrap table{margin:0}.checkout-card{border:2px solid var(--success);background:#fbfffc}.checkout-card input,.checkout-card select{max-width:100%}.complete-sale-btn{width:100%;font-size:17px;padding:15px;background:var(--success);position:sticky;bottom:0}.quick-summary{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin:10px 0 14px}.quick-summary .mini-card{background:#eef6ff;padding:10px;border-radius:12px;text-align:center;font-weight:700}.step-box{border:1px solid #e2e8f0;border-radius:16px;padding:18px;margin-bottom:20px;background:white}.selection-grid{display:flex;flex-wrap:wrap;gap:12px}.selection-btn{border:none;border-radius:14px;padding:16px 22px;min-width:160px;font-weight:bold;cursor:pointer;transition:.2s;color:white}.product-card{border:1px solid #e2e8f0;border-radius:16px;padding:18px;background:white;transition:.2s}.product-card:hover,.pos-item-card:hover{box-shadow:0 6px 20px #00000012}::-webkit-scrollbar{width:8px;height:8px}::-webkit-scrollbar-thumb{background:#94a3b8;border-radius:10px}.thermal-receipt{max-width:320px;margin:auto;font-family:Arial,monospace;font-size:12px;color:#000;background:white}.thermal-receipt table{font-size:11px}.thermal-receipt hr{border:0;border-top:1px dashed #000}
.dark{background:#0f172a;color:#e5e7eb;min-height:100vh}.dark .card{background:#111827;color:#e5e7eb;box-shadow:0 4px 15px #0006}.dark table{background:#111827}.dark td{border-color:#374151}.dark input,.dark select,.dark textarea{background:#0b1220;color:#e5e7eb;border-color:#475569}.dark .small,.dark .nav-group-title{color:#cbd5e1}.dark .header{background:linear-gradient(90deg,#020617,#1e3a8a)}.dark .stat{background:#111827}.dark .nav-section{border-color:#334155}.dark .checkout-card{background:#0b1220}.dark .cart-table-wrap{border-color:#334155}.dark .quick-summary .mini-card{background:#172554}
@media(max-width:1000px){.sales-layout{grid-template-columns:1fr}.checkout-panel{position:relative;top:auto}nav a,.btn{flex:1 1 calc(50% - 10px);text-align:center}.header h1{font-size:28px}.current-sale-box{max-height:300px}}
@media(max-width:560px){.container{padding:12px}.card{padding:14px}.header{padding:18px}.header h1{font-size:22px}nav a,.btn{flex:1 1 100%;font-size:13px}.quick-summary{grid-template-columns:1fr}.cart-table-wrap{max-height:220px}}
@media print{nav,.btn,button,.no-print{display:none!important}.card{box-shadow:none}body{background:white}.header{background:white;color:black;padding:0}.container{padding:0}.thermal-receipt{max-width:80mm}.thermal-receipt *{color:#000!important}}
</style>
"""
...
def page(title, content, **context):
    ...
    <div class="card no-print nav-shell">
        <nav>
            <div class="nav-group-title">Dashboard</div>
            <div class="nav-section">
                <a href="/">🏠 Dashboard</a>
                <a href="/daily-report" class="btn-purple">📊 Daily Report</a>
                <a href="/notifications">🔔 Notifications</a>
                {% if session['role'] == 'manager' %}<a href="/audit-trail" class="btn-gray">🧾 Audit Trail</a>{% endif %}
            </div>

            <div class="nav-group-title">Sales & Customers</div>
            <div class="nav-section">
                <a href="/pos" class="btn-green">💰 Make Sale</a>
                <a href="/sales">🧾 Sales History</a>
                <a href="/debtors">👥 Debtors</a>
                <a href="/customer-history" class="btn-gray">📁 Customer History</a>
                <a href="/debtor-reminders" class="btn-gray">⏰ Debtor Reminder</a>
            </div>

            <div class="nav-group-title">Inventory</div>
            <div class="nav-section">
                <a href="/products">📦 Products / Stock</a>
                <a href="/branch-stock">🏬 Branch Stock</a>
                <a href="/lens-powers">🔍 Lens Powers</a>
                <a href="/lens-restock-center" class="btn-green">➕ Lens Restock Center</a>
                <a href="/lens-search">🔎 Lens Search</a>
                {% if session['role'] == 'manager' %}<a href="/generate-power-grid" class="btn-green">🧮 Generate Power Grid</a>{% endif %}
            </div>

            <div class="nav-group-title">Transfers & Patients</div>
            <div class="nav-section">
                <a href="/request-goods" class="btn-green">🔄 Request Goods</a>
                <a href="/goods-requests" class="btn-gray">📥 Goods Requests</a>
                {% if session['role'] == 'manager' %}<a href="/smart-transfer" class="btn-gray">⚡ Smart Transfer</a>{% endif %}
            ...
            <div class="nav-section">
                ...
                <a href="/security-settings" class="btn-gray">🔒 Security</a>
                <a href="/backup-center" class="btn-gray">💾 Backup Center</a>
                <a href="/office-settings" class="btn-gray">⚙️ Office Settings</a>
                <a href="/appearance" class="btn-gray">🎨 Appearance</a>
            </div>
            {% endif %}
            <div class="nav-section"><a href="/logout" class="btn-red">🚪 Logout</a></div>
        </nav>
    </div>
    {% endif %}
    """
...
def ensure_schema():
    ...
    try:
        ...
        db.session.execute(text("ALTER TABLE hol_product ADD COLUMN IF NOT EXISTS end_user_price FLOAT DEFAULT 0"))
...
def setup_database():
    ...
    try:
        Product.query.filter((Product.end_user_price.is_(None)) | (Product.end_user_price == 0)).update({Product.end_user_price: Product.retail_price}, synchronize_session=False)
    except Exception:
        db.session.rollback()
    db.session.commit()
...
<form method="post">
    ...
    <input name="end_user_price" type="number" step="0.01" placeholder="End User Price" value="{{ product.end_user_price if product else '' }}"><br><br>
...
def add_product():
    ...
    if request.method == "POST":
        product = Product(branch_id=int(request.form.get("branch_id")), name=request.form.get("name"), category=request.form.get("category"), subcategory=request.form.get("subcategory"), retail_price=float(request.form.get("retail_price") or 0), wholesale_price=float(request.form.get("wholesale_price") or 0), end_user_price=float(request.form.get("end_user_price") or 0), quantity=int(request.form.get("quantity") or 0))
    ...
    <div class="card">
        ...
        {% for b in branches %}
            <div class="card" style="background:#f8fafc">
                ...
                <table>
                    <tr><th>Product</th><th>Category</th><th>Subcategory</th><th>Qty</th><th>Wholesale</th><th>Retail</th><th>End User</th></tr>
                    {% for p in products_by_branch[b.id] %}
                    <tr><td>{{ p.name }}</td><td>{{ p.category }}</td><td>{{ p.subcategory }}</td><td>{% if p.quantity <= 5 %}<span class="badge-low">LOW</span> {% endif %}{{ p.quantity }}</td><td>{{ money(p.wholesale_price) }}</td><td>{{ money(p.retail_price) }}</td><td>{{ money(p.end_user_price) }}</td></tr>
                    {% else %}<tr><td colspan="7">No product in this branch.</td></tr>{% endfor %}
                </table>
...
def products():
    ...
    {% endif %}
    <table>
        <tr><th>Branch</th><th>Name</th><th>Category</th><th>Subcategory</th><th>Wholesale</th><th>Retail</th><th>End User</th><th>Qty</th><th>Action</th></tr>
        {% for p in product_list %}<tr><td>{{ branches_map.get(p.branch_id, '') }}</td><td>{{ p.name }}</td><td>{{ p.category }}</td><td>{{ p.subcategory }}</td><td>{{ money(p.wholesale_price) }}</td><td>{{ money(p.retail_price) }}</td><td>{{ money(p.end_user_price) }}</td><td>{% if p.quantity <= 5 %}<span class="badge-low">LOW</span> {% endif %}{{ p.quantity }}</td>{% if session['role']=='manager' %}<td style="min-width:260px"><a href="/edit-product/{{ p.id }}">Edit</a> | <a href="/restock-product/{{ p.id }}">Restock</a><br><form method="post" action="/delete-product/{{ p.id }}" style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap;align-items:center" onsubmit="return confirm('Delete this product permanently? This is for products that are out of market. Sales history will remain, but the product and its lens powers will be removed.');"><input type="hidden" name="return_branch_id" value="{{ selected_branch or '' }}"><input type="password" name="manager_pin" placeholder="Manager PIN" required style="max-width:120px;padding:8px"><button class="btn-red" type="submit" style="padding:8px 10px">Delete Product</button></form></td>{% endif %}</tr>{% endfor %}
...
def delete_product(product_id):
    if not logged_in():
        return redirect(url_for("login"))
    if not is_manager():
        return page("Manager Access Required", "<div class='card'><h2>Manager Access Required</h2><p>Only manager/admin can delete products that are out of market.</p><a class='btn' href='/products'>Back</a></div>")
    if not check_manager_pin(request.form.get("manager_pin")):
        return page("Wrong Manager PIN", "<div class='card danger'><h2>Wrong Manager PIN</h2><p>Product deletion was blocked for security.</p><a class='btn' href='/products'>Back</a></div>")

    product = Product.query.get_or_404(product_id)
    product_name = product.name
    product_branch = product.branch_id

    # Remove linked lens powers first so the product can be deleted safely.
    deleted_lens_count = LensPower.query.filter_by(product_id=product.id).delete(synchronize_session=False)

    # Keep old branch request history, but detach it from the deleted product.
    try:
        GoodsRequest.query.filter_by(product_id=product.id).update({GoodsRequest.product_id: None}, synchronize_session=False)
    except Exception:
        db.session.rollback()
        deleted_lens_count = LensPower.query.filter_by(product_id=product.id).delete(synchronize_session=False)
    ...
    db.session.commit()
    audit_log("Product Deleted", f"Deleted out-of-market product: {product_name}. Branch ID: {product_branch}. Linked lens powers removed: {deleted_lens_count}")

    return_branch_id = request.form.get("return_branch_id")
    if return_branch_id:
        return redirect(url_for("products", branch_id=return_branch_id))
    return redirect(url_for("products"))
...
def edit_product(product_id):
    ...
    if request.method == "POST":
        ...
        product.end_user_price = float(request.form.get("end_user_price") or 0)
...
def pos():
    ...
    price_type = request.values.get("price_type") or customer.get("price_type") or "end_user"
    ...
    if request.method == "POST":
        ...
        price_type = request.form.get("price_type") or "end_user"
        ...
        if view == "lenses":
            ...
            if lens_power_id:
                ...
                price = product.wholesale_price if price_type == "wholesale" else (product.retail_price if price_type == "retail" else product.end_user_price)
    ...
    lens_quick_options = [
        ("Single Vision Lens", "White Lens", "SV White"),
        ("Single Vision Lens", "Photo AR", "SV Photo AR"),
        ("Single Vision Lens", "Blue Cut Photo AR", "SV Blue Cut Photo AR"),
        ("Bifocal Lens", "White Lens", "BF White"),
        ("Bifocal Lens", "Photo AR", "BF Photo AR"),
        ("Bifocal Lens", "Blue Cut Photo AR", "BF Blue Cut Photo AR"),
        ("Progressive Lens", "White Lens", "Progressive White"),
        ("Progressive Lens", "Photo AR", "Progressive Photo AR"),
        ("Progressive Lens", "Blue Cut Photo AR", "Progressive Blue Cut Photo AR"),
    ]
    ...
    <div class="card">
        ...
        <select id="price_type" onchange="reloadPOS()">
            <option value="end_user" {% if price_type == 'end_user' %}selected{% endif %}>End User</option>
    ...
    <div class="sales-layout">
        <div class="sales-left">
            ...
            {% if view == 'lenses' %}
            <div class="card"><h3>Step 2: Select Lens Type</h3>
                <p class="small">Click the exact lens type first. The matching powers will open under it for easy selection.</p>
                {% for category_value, material_value, label in lens_quick_options %}
                <a class="pos-small-btn {% if lens_category == category_value and lens_material == material_value %}active{% endif %}" onclick="return posLink(this)" href="/pos?view=lenses&lens_category={{ category_value }}&lens_material={{ material_value }}">{{ label }}</a>
                {% endfor %}
            </div>

            {% if lens_category and lens_material %}
            <div class="card">
                <h3>Step 3: Select Power</h3>
                <p class="small">Only {{ lens_category.replace(' Lens','') }} - {{ lens_material }} powers will show here. Pick the power from the dropdown and add it to the cart.</p>
                <div class="pos-card-grid">
                    {% for p in product_list %}
                    <form class="pos-item-card lens-product-card saleForm" method="post" onsubmit="copyCustomerToForm(this)">
                        ...
                        <p class="small">Wholesale ₦{{ money(p.wholesale_price) }} | Retail ₦{{ money(p.retail_price) }} | End User ₦{{ money(p.end_user_price) }}</p>
            ...
            <form class="card saleForm" method="post" onsubmit="copyCustomerToForm(this)">
                ...
                <h3>Add Frames, Cases, Cleaners and Cloths</h3>
                <p class="small">Frames and accessories load directly for fast selection. Enter quantity beside each item and add selected products to the same customer receipt.</p>
                <div class="pos-card-grid">
                    {% for p in product_list %}
                    <div class="pos-item-card product-card"><h3>{{ p.name }}</h3><p>{{ p.category }} - {{ p.subcategory }}</p><p><b>Stock:</b> {{ p.quantity }} | {{ stock_badge(p.quantity) }}</p><p class="small">Wholesale ₦{{ money(p.wholesale_price) }} | Retail ₦{{ money(p.retail_price) }} | End User ₦{{ money(p.end_user_price) }}</p><input name="qty_{{ p.id }}" type="number" min="0" placeholder="Quantity"></div>
    ...
    function customerParams(){
        ...
        params.set('price_type', document.getElementById('price_type').value || 'end_user');
    ...
    function copyCustomerToForm(form){
        ...
        form.querySelector('input[name="price_type"]').value = document.getElementById('price_type').value || 'end_user';
    }
    // Lens search and Recent Lens Sales were removed to keep Make Sale simple for new users.
    // Frames, cases, cleaners, cloths and other accessories now load directly for selection.
    </script>
    """, view=view, lens_category=lens_category, lens_material=lens_material, lens_quick_options=lens_quick_options, product_list=product_list, lens_map=lens_map, lens_stock_map=lens_stock_map, cart=cart, cart_total=cart_total, customer_name=customer_name, customer_phone=customer_phone, price_type=price_type, selected_branch_id=selected_branch_id, short_lens_name=short_lens_name, stock_badge=stock_badge)
...
def pos_checkout():
    ...
    session["pos_cart"] = []
    session["pos_customer"] = {"customer_name": "", "customer_phone": "", "branch_id": session.get("branch_id"), "price_type": "end_user"}
    session.modified = True
...
def receipt(sale_id):
    ...
    return page("Receipt", """
    <div class="card"><center><h2>HALLELUYAH OPTICAL LABORATORY</h2><h3>Receipt / Invoice</h3></center>
...
def download_products_backup():
    ...
    rows = [(p.branch_id, p.name, p.category, p.subcategory, p.wholesale_price, p.retail_price, p.end_user_price, p.quantity) for p in products]
    return csv_response(f"HOL_products_backup_{datetime.now().strftime('%Y%m%d')}.csv", "Branch ID,Product Name,Category,Subcategory,Wholesale Price,Retail Price,End User Price,Quantity", rows)
...
def approve_goods_request(request_id):
    ...
    if not destination_product:
        destination_product = Product(
            ...
            end_user_price=source_product.end_user_price,
            quantity=0
        )
