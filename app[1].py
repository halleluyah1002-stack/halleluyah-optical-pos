...
def branch_stock():
    ...
    branch_data = []
    for b in branches:
        products = Product.query.filter_by(branch_id=b.id).order_by(Product.category, Product.subcategory, Product.name).all()
        lenses = LensPower.query.filter_by(branch_id=b.id).order_by(LensPower.sph, LensPower.cyl, LensPower.add_power).all()
        product_count = len(products)
        lens_power_count = len(lenses)
        total_product_qty = sum(int(p.quantity or 0) for p in products)
        total_lens_qty = sum(int(l.quantity or 0) for l in lenses)
        low_products = [p for p in products if int(p.quantity or 0) <= 5]
        low_lenses = [l for l in lenses if int(l.quantity or 0) <= 2]
        branch_data.append({
            "branch": b,
            "products": products,
            "lenses": lenses,
            "product_count": product_count,
            "lens_power_count": lens_power_count,
            "total_product_qty": total_product_qty,
            "total_lens_qty": total_lens_qty,
    ...
    <div class="card">
        <h2>Branch Stock Separation</h2>
        <p class="small">Manager view: products are separated branch by branch for easy identification. Each branch has its own summary, products table and lens power table.</p>

        <div class="grid">
        {% for data in branch_data %}
            <div class="card stat">
                <h3>{{ data.branch.name }}</h3>
                <p class="small">{{ data.branch.address }}</p>
                <p><b>Products:</b> {{ data.product_count }} | <b>Product Qty:</b> {{ data.total_product_qty }}</p>
                <p><b>Lens Powers:</b> {{ data.lens_power_count }} | <b>Lens Qty:</b> {{ data.total_lens_qty }}</p>
                {% if data.low_products or data.low_lenses %}
                    <p><span class="badge-low">LOW STOCK ALERT</span></p>
                {% else %}
                    <p><span style="background:#198754;color:white;border-radius:12px;padding:3px 8px;font-size:12px">OK</span></p>
                {% endif %}
            </div>
        {% endfor %}
        </div>
    ...
    {% for data in branch_data %}
    <div class="card" style="border-left:8px solid #0077b6">
        <h2>{{ data.branch.name }}</h2>
        <p class="small">{{ data.branch.address }}</p>

        <div class="grid">
            <div class="card stat"><h3>Product Types</h3><h1>{{ data.product_count }}</h1></div>
            <div class="card stat"><h3>Total Product Qty</h3><h1>{{ data.total_product_qty }}</h1></div>
            <div class="card stat"><h3>Lens Powers</h3><h1>{{ data.lens_power_count }}</h1></div>
            <div class="card stat"><h3>Total Lens Qty</h3><h1>{{ data.total_lens_qty }}</h1></div>
        </div>

        <h3>Products / Frames / Accessories in {{ data.branch.name }}</h3>
        <table>
            <tr><th>Product</th><th>Category</th><th>Subcategory</th><th>Qty</th><th>Retail</th><th>Wholesale</th><th>Action</th></tr>
            ...
            <tr>
                ...
                <td><a href="/edit-product/{{ p.id }}">Edit</a> | <a href="/restock-product/{{ p.id }}">Restock</a></td>
            </tr>
            {% else %}
            <tr><td colspan="7">No product in this branch.</td></tr>
            {% endfor %}
        </table>

        <h3 style="margin-top:25px">Lens Power Stock in {{ data.branch.name }}</h3>
        <table>
            <tr><th>Lens Product</th><th>SPH</th><th>CYL</th><th>AXIS</th><th>ADD</th><th>Qty</th><th>Action</th></tr>
            {% for l in data.lenses %}
            <tr>
                <td><b>{{ products_map.get(l.product_id, '') }}</b></td>
                <td>{{ l.sph }}</td>
                <td>{{ l.cyl }}</td>
                <td>{{ l.axis }}</td>
                <td>{{ l.add_power }}</td>
                <td>{% if l.quantity <= 2 %}<span class="badge-low">LOW</span> {% endif %}{{ l.quantity }}</td>
                <td><a href="/restock-lens/{{ l.id }}">Restock</a></td>
            </tr>
            {% else %}
            <tr><td colspan="7">No lens power stock in this branch.</td></tr>
            {% endfor %}
        </table>
    </div>
    {% endfor %}
    """, branch_data=branch_data, products_map=products_map)
...
def products():
    ...
    if is_manager():
        # Manager sees products separated by branch for easy identification.
        if selected_branch:
            visible_branches = [b for b in branches if b.id == int(selected_branch)]
        else:
            visible_branches = branches
    else:
        selected_branch = str(session.get("branch_id"))
        visible_branches = [b for b in branches if b.id == int(session.get("branch_id") or 0)]

    products_by_branch = {}
    branch_summary = {}
    for b in visible_branches:
        rows = Product.query.filter_by(branch_id=b.id).order_by(Product.category, Product.subcategory, Product.name).all()
        products_by_branch[b.id] = rows
        branch_summary[b.id] = {
            "count": len(rows),
            "qty": sum(int(p.quantity or 0) for p in rows),
            "low": sum(1 for p in rows if int(p.quantity or 0) <= 5),
        }

    return page("Products", """
    <div class="card">
        <h2>Products / Stock By Branch</h2>
        <p class="small">Products are separated by branch so manager can easily identify where every item belongs.</p>
    ...
    <div class="card" style="border-left:8px solid #0077b6">
        ...
        <div class="grid">
            <div class="card stat"><h3>Product Types</h3><h1>{{ branch_summary[b.id].count }}</h1></div>
            <div class="card stat"><h3>Total Quantity</h3><h1>{{ branch_summary[b.id].qty }}</h1></div>
            <div class="card stat"><h3>Low Stock Items</h3><h1>{{ branch_summary[b.id].low }}</h1></div>
        </div>

        <table>
            <tr><th>Product</th><th>Category</th><th>Subcategory</th><th>Retail</th><th>Wholesale</th><th>Qty</th><th>Action</th></tr>
            ...
            <tr>
                ...
                <td>
                    {% if session['role']=='manager' %}
                    <a href="/edit-product/{{ p.id }}">Edit</a> | <a href="/restock-product/{{ p.id }}">Restock</a>
                    {% else %}
                    View Only
                    {% endif %}
                </td>
            ...
            {% else %}
            <tr><td colspan="7">No product found in this branch.</td></tr>
            {% endfor %}
        </table>
    </div>
    {% endfor %}
    <a class="btn" href="/">Back</a>
    """, branches=branches, visible_branches=visible_branches, products_by_branch=products_by_branch, branch_summary=branch_summary, branches_map=branches_map, selected_branch=selected_branch)
